...
@objc
protocol ICADKGroupDataManagerObserver {
	
	@objc optional func groupDidUpdate(_ groupId: String)
	@objc optional func groupDidRemove(_ groupId: String)
	@objc optional func group(_ groupId: String, didRemoveAssets assets: [ICADKAsset])
	@objc optional func group(_ groupId: String, didInsertAssets assets: [ICADKAsset])
    @objc optional func groupDidUpdateComplete(_ groupId: String)
}

public class ICADKGroupDataManager: ICADKBaseManager, PHPhotoLibraryChangeObserver {

    public var groupIds: [String]?
	private var groups: [String : ICADKAssetGroup]?
    private var assets = [String: ICADKAsset]()
	...
	public func fetchGroups(_ completeBlock: (_ groups: [String]?, _ error: NSError?) -> Void) {
		if let assetGroupTypes = self.assetGroupTypes {
			...
			var groups: [String : ICADKAssetGroup] = [:]
			var groupIds: [String] = []
			...
			for (_, groupType) in assetGroupTypes.enumerated() {
                ...
                fetchResult.enumerateObjects({ (collection, index, stop) in
                    let assetGroup = ICADKAssetGroup()
	...
	public func fetchGroupWithGroupId(_ groupId: String) -> ICADKAssetGroup {
	...
	public func fetchGroupThumbnailForGroup(_ groupId: String, size: CGSize, options: PHImageRequestOptions, completeBlock: @escaping (_ image: UIImage?, _ info: [AnyHashable: Any]?) -> Void) {
		...
		let latestAsset = ICADKAsset(originalAsset:group.fetchResult.firstObject!)
		latestAsset.fetchImageWithSize(size, options: options, completeBlock: completeBlock)
	}
	
	public func fetchAssetWithGroup(_ group: ICADKAssetGroup, index: Int) -> ICADKAsset {
        let originalAsset = group.fetchResult[index]
        var asset = self.assets[originalAsset.localIdentifier]
        if asset == nil {
            asset = ICADKAsset(originalAsset:originalAsset)
	...
	private func updateGroup(_ group: ICADKAssetGroup, collection: PHAssetCollection) {
	...
	private func updateGroup(_ group: ICADKAssetGroup, fetchResult: PHFetchResult<PHAsset>) {
        ...
        for group in self.groups!.values {
			if let changeDetails = changeInstance.changeDetails(for: group.originalCollection) {
				if changeDetails.objectWasDeleted {
					...
					self.notifyObserversWithSelector(#selector(ICADKGroupDataManagerObserver.groupDidRemove(_:)), object: group.groupId as AnyObject?)
					continue
				}
				...
				if let objectAfterChanges = changeDetails.objectAfterChanges as? PHAssetCollection {
					...
					self.notifyObserversWithSelector(#selector(ICADKGroupDataManagerObserver.groupDidUpdate(_:)), object: group.groupId as AnyObject?)
				}
			}
			
			if let changeDetails = changeInstance.changeDetails(for: group.fetchResult) {
                let removedAssets = changeDetails.removedObjects.map{ ICADKAsset(originalAsset: $0) }
				if removedAssets.count > 0 {
					self.notifyObserversWithSelector(#selector(ICADKGroupDataManagerObserver.group(_:didRemoveAssets:)), object: group.groupId as AnyObject?, objectTwo: removedAssets as AnyObject?)
				}
				self.updateGroup(group, fetchResult: changeDetails.fetchResultAfterChanges)
				
                let insertedAssets = changeDetails.insertedObjects.map{ ICADKAsset(originalAsset: $0) }
				if insertedAssets.count > 0  {
					self.notifyObserversWithSelector(#selector(ICADKGroupDataManagerObserver.group(_:didInsertAssets:)), object: group.groupId as AnyObject?, objectTwo: insertedAssets as AnyObject?)
				}
                
                self.notifyObserversWithSelector(#selector(ICADKGroupDataManagerObserver.groupDidUpdateComplete(_:)), object: group.groupId as AnyObject?)
			}
		}
	}
