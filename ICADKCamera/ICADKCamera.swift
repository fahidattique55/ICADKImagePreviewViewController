...
open class ICADKCameraPassthroughView: UIView {
...
@objc
public enum ICADKCameraDeviceSourceType : Int {
    case front, rear
}

open class ICADKCamera: UIViewController, AVCaptureMetadataOutputObjectsDelegate {
    ...
    open var currentDeviceType = ICADKCameraDeviceSourceType.rear
    ...
    open lazy var flashButton: UIButton = {
        ...
        flashButton.addTarget(self, action: #selector(ICADKCamera.switchFlashMode), for: .touchUpInside)
    ...
    open func setupUI() {
        ...
        let cameraSwitchButton: UIButton = {
            let cameraSwitchButton = UIButton()
            cameraSwitchButton.addTarget(self, action: #selector(ICADKCamera.switchCamera), for: .touchUpInside)
            cameraSwitchButton.setImage(ICADKCameraResource.cameraSwitchImage(), for: .normal)
        ...
        let captureButton: UIButton = {
            ...
            captureButton.addTarget(self, action: #selector(ICADKCamera.takePicture), for: .touchUpInside)
        ...
        contentView.addGestureRecognizer(UIPinchGestureRecognizer(target: self, action: #selector(ICADKCamera.handleZoom(_:))))
        contentView.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(ICADKCamera.handleFocus(_:))))
    }
...
public extension Bundle {
    ...
    class func cameraBundle() -> Bundle {
        let assetPath = Bundle(for: ICADKCameraResource.self).resourcePath!
        return Bundle(path: (assetPath as NSString).appendingPathComponent("ICADKCameraResource.bundle"))!
    }
    
}

open class ICADKCameraResource {
